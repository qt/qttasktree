...
static void autoSyncOn(QPromise<void> &promise)
{
    ...
    QThread::sleep(1ms); // Spin for a while so that QConcurrentCallBase::syncAll() awaits for us.
    s_syncDone = true;
}
...
static void autoSyncOff(QPromise<void> &promise)
{
    ...
    while (!promise.isCanceled())
        QThread::sleep(1ms); // Spin for a while so that ~QConcurrentCall awaits for us.
    s_syncDone = true;
}
